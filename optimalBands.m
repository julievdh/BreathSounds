...
highc = 2000-width:100:10000;
for i = 1:length(lowc) % all low bands
    for j = 1:length(highc) % across all high bands
        for k = 1:size(Pxxo,2)
            Pxxlow_o(k) = mean(Pxxo(F>lowc(i)-width/2 & F <lowc(i)+width/2,k));
            Pxxhigh_o(k) = mean(Pxxo(F>highc(j)-width/2 & F <highc(j)+width/2,k));
        end
        if exist('minflow') == 1
        [~,RES(j,i)] = evalcorr(Pxxhigh_o(pon3)-Pxxlow_o(pon3),[minflow{:}],0,'g',0);
        else 
            [~,RES(j,i)] = evalcorr(Pxxhigh_o(pon3)-Pxxlow_o(pon3),all_SUMMARYDATA(pnum,4)',0,'g',0);
        end
    end
end
...
for k = 1:size(SL_o,2)
    Pxxlow_o(k) = mean(SL_o(F>bestlow-width/2 & F <bestlow+width/2,k));
    Pxxhigh_o(k) = mean(SL_o(F>besthigh-width/2 & F <besthigh+width/2,k));
